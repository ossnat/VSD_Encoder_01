#!/usr/bin/env python3
"""Sweep CORnet-S cortical layers and summarize metrics + weight/alpha maps.

Default layers: V1_pool7, V1_pool14, V2, V4 (validation selection split).

Example (local smoke — V4 only):
  scripts/py scripts/11_sweep_cornet_layers.py \\
    --window configs/windows/evoked_35_42.yaml \\
    --layers V4 --device cpu

Full comparison:
  scripts/py scripts/11_sweep_cornet_layers.py \\
    --window configs/windows/evoked_35_42.yaml \\
    --layers V1_pool7 V1_pool14 V2 V4 --device cpu
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--layers",
        nargs="+",
        default=DEFAULT_CORNET_LAYERS,
        help="CORnet taps to sweep (default: V1_pool7 V1_pool14 V2 V4)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Center-disk radius as fraction of eval-mask radius for weight summaries",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Reuse the generic layer-sweep driver for extract→train→eval.
    ...
    # Keep CORnet outputs under a dedicated subfolder for clarity.
    report_dir = comparison_output_dir(repo, cfg, window_id) / "cornet_s_comparison"
    ...
    # Weight / alpha spatial comparison (requires trained ridge models).
    try:
    ...
    # Also copy metrics into the generic layer_sweep dir used by other tools.
    out_dir.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_dir / "layer_comparison.csv", index=False)

    print(f"\nCORnet layer comparison complete for {slug}")
    if not df.empty:
        cols = [
            "feature_layer",
            "feature_shape",
